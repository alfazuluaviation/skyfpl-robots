# 🚁 SkyFPL - Robô Processador de Rotas Especiais de Helicópteros (REAH)

Este robô é responsável por coletar, georreferenciar, mesclar e otimizar as **cartas brasileiras de Rotas Especiais de Helicópteros (REAH)** do GeoServer oficial do DECEA via WMS, convertendo-as em arquivos SQLite `.mbtiles` ultra-leves e eficientes no Cloudflare R2.
...
5.  **Telemetria em Tempo Real (R2 Pipe):** Atualiza um status JSON (`telemetry.json` em `charts/reah/`) a cada progresso incremental com porcentagens e metadados de tamanho físico, permitindo monitoramento remoto em tempo real pelo Dashboard SkyNav Pro.

---

## 🇧🇷 Grade de Cartas Cobertas (REAH)

O robô possui um inventário estático das coordenadas limite exatas ($BBOX$) mapeadas do DECEA GetCapabilities:

*   **REAH Rio de Janeiro** (`REH_WJ2_RIO_DE_JANEIRO`)
*   **REAH São Paulo** (`REH_XP_SAO_PAULO`)
*   **REAH Vitória** (`REH_XR_VITORIA`)
...
python reah-robot/build_reah.py
```
